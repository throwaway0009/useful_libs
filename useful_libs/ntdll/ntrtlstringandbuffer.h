...
// A buffer that has been freed is in the same state as one that just been _initialised.
// A buffer that is all zeros (RtlZeroMemory) can be freed. You do not need to
//   track if you _initialised it.
//--
...
// multiple buffers, as long as the buffers are actually _initialised
